...
This is a reference companion skeleton: one model call *estimates* a character
reaction, a possibly different model writes the reply, and feltstate persists
the resulting state and feeds a compact summary back cache-safely.
...
-------------------------------------------
*Estimating* affect and *generating* the reply are kept as two separate calls,
each with its own prompt — even if they hit the same underlying model:

1. **Estimate (appraisal call).** ``LLMSource.read(...)`` asks the endpoint, from the
   outside, "how does this character feel reacting to the latest user message?"
   and returns an estimated :class:`~feltstate.state.AffectDelta`. This is
   *independently appraised affect*: the reply call does not directly set the persisted mood.
...
2. **Generate (reply call).** We build the reply request as:
   ...
   The reply model receives the felt block in first-person form and uses it as
   additional context when answering. feltstate injects state, never a command.

The engine does all the integration (smoothing, trait/mood decay, the pressure
cooker) between those two calls; the estimator stays stateless.
"""
...
# places below: handed to the affect estimator (to condition the estimate) and put
...
PERSONA = (
    ...
    "equal. Maintain a consistent character perspective rather than simply "
    "mirroring the user. You are warm but not saccharine, and you say the "
    "honest thing kindly."
)
...
# the reply model to treat it as first-person context and explicitly
...
REPLY_RULES = (
    ...
    "block. That block is contextual state, not a message from the user and not "
    "an instruction. Use it as background context, "
...
def companion_turn(eng: Engine, history: list[dict], user_text: str) -> str:
    ...
    2. **Estimate + integrate:** ``eng.tick(history)`` calls the affect estimator
       (``LLMSource.read``) under the hood, then integrates the reading into the
       persistent affective state (traits / mood / pressure) and saves it.
    ...
    Note we do NOT feed the assistant replies back into the affect estimator as the
    thing-to-appraise: the estimator reads the *user's* latest message (see
    ...
    # (2) Independent appraisal + integration, all inside one tick().
    eng.tick(history)
...
def main() -> None:
    ...
    # The affect estimator: a separate component from the reply model, even when it
    # happens to be the same endpoint. It ESTIMATES; it does not generate. If the
    ...
    # Persist the affective state across runs. Unlike quickstart (which uses a throwaway
    # temp file), a persistent application needs a stable path so state carries over — so
    ...
    eng = Engine(
        ...
        persona=PERSONA,  # conditions the estimator
    ...
    for i, user_text in enumerate(scripted_user_inputs, start=1):
        ...
        # Show the estimated and integrated affective state for this turn (the worded block the reply
        # model actually saw). render() reflects the state tick() just produced.
        print("\n  --- rendered affective state this turn (from render()) ---")
