...
"You are feeling sad and guarded; respond accordingly." Don't. The production
system this was distilled from tried versions of that and the lesson was blunt:
**injecting behavioural rules makes the model worse** — flatter, more
performative, more obviously following a script. A whole subsystem that
auto-injected "remember to mention X" / "don't repeat yourself" rules was
eventually ripped out for exactly this reason.
...
companion* lives or dies on it, and shipping the state loop without it would be
shipping a money fire.

---

## 6. What's new, what's not

Said plainly, so nobody oversells it:
...
The value of open-sourcing this isn't "we invented affective computing." It's
"here is that specific, opinionated whole, as code you can run and argue with."
