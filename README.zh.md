...
**给 LLM agent 一份能带着走的心情——和会老化、会改写、会死去、还会彼此相认的记忆。**
...
参考库, 给长期运行的 agent 一副持久且*可调*的性格。它由**三大系统咬合而成,
外加一张把记忆织在一起的网**:

- **一条记忆生产线。** 底层是从不改写的逐字对话原文; 提炼成紧凑的 5W1H 事实,
  显著度会衰减、因重复而加强、被想起时活得更久; 出生到死亡全程可审计的
  生命周期 (出生指纹、融合谱系、墓碑先行的删除、哈希链审计); 召回是 agent
  自己推开的门, 从不偷偷注入。
- **一台感情引擎。** 在回复模型*之外*判读——模型无权书写自己的持久感受——并在
  多个时间尺度上整合: 快的心情、会泄压的压力条、慢的性情、永久印痕, 以及约
  180 天纲的可塑层。没人说话时它也在走。
- **一套时间纪律。** 时间戳只出自程序、从不出自模型的文字; 事实带双时间
  (事情*发生*的时刻 vs 被*记下或修正*的时刻), 历史可以按"当时相信什么"回查。
- **一张键网。** 出生时印在事实上的单词键, 加上一对一判过票的亲缘边, 把平铺的
  账本织成网。查询靠词与键相撞入场, 顺判过的边拢亲戚, 读链尾即当下——没有
  向量索引, 也没有作废判决机 (见[键网](#键网-记忆的横向维度))。

有个立场先摆在前面: 驱动这一切的动力学常数——衰减速率、压力阈值、可塑曲线——
是**性格与韧性参数, 有意做成可塑的**。重调它们就是写出*另一个*角色的方式。
它们不是科学测量值, 也不作这种主张 (见["数值从哪来"](#数值从哪来))。

引擎之外还配了一套[集成手册](#集成手册): 十六章讲这些零件如何装配成一个
会说话、有脸、会干活、会出错、还保持住自己的陪伴体——手册里引用的每段对话
都能用可运行的示例复现。

它借鉴了情感计算、agent 记忆、appraisal 理论与选择性遗忘的研究, 但架构是
自己的。其中几样在 agent 库里并不常见——能*死亡*且留下可追谱系的记忆、持久
感情的硬所有权边界、判出来 (而非嵌入出来) 的关系网、离线零 LLM 的做梦——
它们是有意做成这样的, 不是现成零件的拼装。
...
1. **记忆有生命周期, 不只是检索索引。** `Canon` 存储紧凑的 5W1H 事实, 显著度会
   ...
   发现无解释变动或消失的哈希链审计账本。
2. **记忆是一张网, 不只是一本账。** 单词键在事实出生时印上行; 每条新事实对
   全账做一次撞键碰撞, 由裁判决定亲不亲, `relates` 边连同*为什么*落在两行
   之上。查询腿 (`Canon.reach`) 撞键入场、顺判过的边拢亲戚, 按事发时间排成
   链——**链尾即当下**: 新的第一手事实靠站在最后胜出, 不靠作废标记。
3. **时间是纪律, 不是装饰。** 时间戳由程序盖; 事实是双时间的 (`valid_at` 对
   记录时刻, 信念终止时有 `invalid_at`), 历史可按过去某一刻"当时相信什么"
   回查, 键网给候选定价用的也是事情*发生*时刻之间的差。
4. **感情是独立估计的, 不是自我报告。** 由配置的 `AffectSource` 产出感情信号,
   回复模型无法随意书写自己的持久状态。
5. **状态在多个时间尺度上演化。** 快的心情、慢的性情、关系、压力、余味、期待
   与可选的印痕, 按可配置的、类人的不对称动力学整合——每个速率都是性格旋钮,
   不是拟合出来的常数 (见["数值从哪来"](#数值从哪来))。最慢的一层是**可塑性**:
   每根压力条的敏感度被真实经历的命中微量刻蚀, 放大后续充压, 又按 safety
   决定的日百分比向基线愈合——性格改变以约 180 天为纲
   (`examples/plasticity.py`)。
6. **能力与心情分开评价。** 可选的技能记忆区用人工 1/2/3 评分、晋升、退役与
   有界探索, 而不是让回复模型自行宣称胜任。
7. **主动行为是被门控的, 不只是被调度的。** 参考调度器把空闲时间、在场、冷却、
   每日配额、待聊话题、时间窗、做梦、内省与日记行为组合进
   propose/dispatch/commit 流程。
8. **持久状态是上下文, 永远不是命令。** 回复模型收到的是紧凑的第一人称描述,
   而不是数值控制或"表现得悲伤一点"这类指令。
9. **注入路径对缓存友好。** 静态人格文本保持稳定前缀, 动态状态搭在最新一条
   用户消息上。
10. **做梦是可选的状态实验。** 零 LLM 地重组带感情标签的碎片, 留下一缕临时的
...
本身就是设计, 不是免责声明。合起来看, `config.py` + `PersonaDials` 就是
**性格设置器**——旋钮是你*写*一个角色的方式, 引擎是让这个角色持续活着的东西。
这些数值唯一不是的, 是对人类数据的拟合; 这里不作这种主张, 它们的职责也
不需要这种主张。
...
打分器召回, 还能展开回产生它的原文对话上下文。(默认打分器按空格分词做词法
重叠——中文等非空格语言在这条缝里换上带分词的打分器即可。)
...
上面的生命周期是记忆的*纵轴*——一条事实从封印出生到留痕死亡的一生。键网是
*横轴*: 散在时间线上的事实如何彼此相认, 而且不靠向量数据库。

两样小东西**长在行上**, 从不放进旁挂索引:

- `keys`——几个说清这条事实关于什么的**单词**。键存在的意义是*相撞*: 两条事实
  共用一个词, 它们就打过照面。词组会被机械地拒掉 (一句话形状的键是指纹,
  独一无二, 因此永远撞不上任何东西), `key_vocab` 会把账本已经在说的词摆给
  提炼器看——新事实优先沿用旧词, 才真的会相遇。
- `relates`——通到别的事实的边, 每条都带着裁判绑定它们的*为什么*。共用词只造出
  *候选*; 只有裁判 (一次 LLM 调用、一套规则、一个人——任何可调用对象) 说这两条
  是亲, 边才落笔, 而且落在**两行**之上。候选是算术, 亲缘是判断; 本库不伪造判断。

进入新事实候选池的资格是挣来的, 不是熬出来的: 候选的*出生*强度乘上一个至多
...
`python examples/key_web.py` (离线、确定性):
...
把链和 lifecycle 的下钻配在一起, 链上任何一条事实都能一路降到产生它的原文
对话现场。
...
- **不是:** 成品。没有捆绑人格、没有训练好的感情分类器、没有对话数据、立绘或
